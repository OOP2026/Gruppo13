...
public class SedutaImplementazionePostgres implements SedutaDAO {
    public ResultSet getTesi(LocalDate data, LocalTime ora, String docente, ConnessioneDatabase conn) {
        try{
            return conn.executeQuery("SELECT 1 FROM Tesi WHERE Tesi.IdTe=(SELECT ID_Te FROM Seduta WHERE Login='"+docente+"'AND Data='"+data+"' AND Ora='"+ora.toString()+"')");
        }
    ...
    public int getVotoFinale(LocalDate data, LocalTime ora, String docente, ConnessioneDatabase conn) {
        try{
            int v=-1;
            ResultSet x = conn.executeQuery("SELECT VotoFinale FROM Seduta WHERE Login='"+docente+"'AND Data='"+data+"' AND Ora='"+ora.toString()+"'");
            if (x.next())
